...
gulp.task(
	'build:app-web-dev',
	gulp.series('clean:public', gulp.parallel('webpack:app-web-dev', 'copy:styles', 'copy:imagemin', 'copy:manifest'))
);

// build app files for web mode in prod
gulp.task(
	'build:app-web-prod',
	gulp.series('clean:public', gulp.parallel('webpack:app-web-prod', 'copy:styles', 'copy:imagemin', 'copy:manifest'))
);
...
gulp.task(
	'default',
	gulp.series('build:app-web-dev', 'browser-sync', 'watch:app-web-files', function runningReload(done) {
...
gulp.task('electron:base-prod', gulp.series('clean:dist', 'build:app-electron-prod', 'build:electron-ts'));

gulp.task('electron:non-release-builder', function (done) {
	const spawnedProcess = spawn(
		/^win/.test(process.platform) ? 'electron-builder.cmd' : 'electron-builder',
...
gulp.task('electron:release-builder', function (done) {
	const spawnedProcess = spawn(
		/^win/.test(process.platform) ? 'electron-builder.cmd' : 'electron-builder',
		['-w', '-p', "'onTagOrDraft'", '--projectDir', '../', '--config', 'config/electron-builder.json'],
		{ stdio: 'inherit' }
	);
	spawnedProcess.on('close', (code) => {
		if (code && code > 0) {
...
gulp.task('electron:prod', gulp.series('electron:base-prod', 'electron:non-release-builder'));

gulp.task('electron:release', gulp.series('electron:base-prod', 'electron:release-builder'));
