...
const webpackHandler = (command, done) => {
	const spawnedProcess = spawn(/^win/.test(process.platform) ? 'npm.cmd' : 'npm', ['run', command], { stdio: 'inherit' });
...
gulp.task('webpack:dev', done => webpackHandler('dev:webpack', done));
// Webpack on Production Mode
gulp.task('webpack:prod', done => webpackHandler('build:webpack', done));
...
gulp.task('electron:watch', function runElectronWatch(done) {
	...
	const spawnedProcess = spawn(/^win/.test(process.platform) ? 'nodemon.cmd' : 'nodemon', ['--exec', 'electron', './public/electron-app.js'], { stdio: 'inherit' });
	spawnedProcess.on('close', code => {
		if (code && code > 0) {
